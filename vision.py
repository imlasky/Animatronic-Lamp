...
import numpy as np
import sys
sys.path.insert(0, '/home/pi/Luxo')
import time
...
class Camera(threading.Thread):
    ...
    def detect_object(self, cascade, feed):
        ...
        while self.flag:
            ...
            feed_by_frame_face = cv2.resize(feed_by_frame, (0,0), fx=0.2, fy=0.2)
            feed_by_frame_paper = cv2.resize(feed_by_frame, (0,0), fx=0.2, fy=0.2)

            # detect which object to detect

            if cascade == 0:
                self.frame_height, self.frame_width = feed_by_frame_face[:,:,0].shape
                gray_feed = cv2.cvtColor(feed_by_frame_face, cv2.COLOR_BGR2GRAY)
                flip_feed_g = cv2.flip(gray_feed, 0)
                self.find_object(gray_feed, feed_by_frame_face, front_face_cascade)
            elif cascade == 1:
                ...
                #gray_feed = cv2.cvtColor(feed_by_frame_paper, cv2.COLOR_BGR2GRAY)
                #self.find_boxes(gray_feed, feed_by_frame)
                self.find_paper(feed_by_frame_paper)
    ...
    def find_paper(self, im):

        lower_bounds = np.array([230, 230, 230])
        upper_bounds = np.array([255, 255, 255])

        mask = cv2.inRange(im, lower_bounds, upper_bounds)
        ...
        if not M["m00"]:
            self.object_coord = self.object_coord
        else:
            cX = int(M["m10"] / M["m00"])
            cY = int(M["m01"] / M["m00"])
            self.object_coord = [cX, cY]
